...
def verifier_stock_et_prix(nom_produit: str):
    ...
    prix_texte = (
        int(premier.prix_promo_produit)
        if premier.prix_promo_produit
        else int(premier.prix_unitaire_produit)
    )
...
def detecter_intention(message: str):
    ...
    if any(
        ...
        for k in [
            ...
            "comment ça marche pour commander",
        ]
    ):
...
def repondre_selon_intention(utilisateur, intention, message):
    ...
    if intention == "statut_commande":
        ...
        if not utilisateur or not getattr(utilisateur, "is_authenticated", False):
            return {
                "reply": "Pour consulter le statut d'une commande, veuillez vous connecter ou fournir la référence de commande (ex: AfriCart-C-20260307-001).",
                "data": None,
            }
        ...
        if m:
                ...
                if commande:
                    data = {
                        ...
                        "total_ttc": (
                            ...
                            if getattr(commande, "total_ttc", None) is not None
                            else None
                        ),
                        "date": (
                            str(commande.date_commande)
                            if getattr(commande, "date_commande", None)
                            else None
                        ),
                    }
                    ...
                    return {
                        "reply": "Référence non trouvée. Vérifiez la référence de commande ou contactez le support.",
                        "data": None,
                    }
            except Exception:
                return {
            ...
            return {
                "reply": "Merci de fournir la référence de commande (ex: AfriCart-C-20260307-001) pour que je puisse vérifier le statut.",
                "data": None,
                "audience": "auth",
            }
    ...
    if intention == "mes_commandes":
        ...
        if not utilisateur or not getattr(utilisateur, "is_authenticated", False):
            return {
                "reply": "Pour consulter vos commandes, veuillez vous connecter à votre compte AfriCart.",
                "data": None,
            }
                ...
                items.append(
                    {
                        ...
                        "total_ttc": (
                            float(c.total_ttc)
                            if getattr(c, "total_ttc", None) is not None
                            else None
                        ),
                        "date": (
                            str(c.date_commande)
                            if getattr(c, "date_commande", None)
                            else None
                        ),
                    }
                )
    ...
    if intention == "infos_paiement":
        return {
            "reply": FAQ_LOCAL["moyens_de_paiement"],
            "data": {"faq": "moyens_de_paiement"},
        }

    if intention == "promotion":
        return {
            "reply": "Les 'Promotions' ne sont pas encore disponibles.",
            "data": None,
        }

    if intention == "salutation":
        return {
            "reply": "Bonjour ! Je suis l'assistant AfriCart — comment puis-je vous aider pour vos achats aujourd'hui ?",
            "data": None,
        }
...
def obtenir_details_produit(nom_produit: str):
    ...
    # Texte de synthèse
    reponse_texte = (
        f"J'ai trouvé {len(produits)} produit(s) correspondant à '{nom_produit}'."
    )
...
def chatbot(request):
    ...
    if est_hors_sujet(message_utilisateur):
        ...
        return Response(
            {"reply": reply_text, "history": historique}, status=status.HTTP_200_OK
        )
    ...
    if reponse_locale:
        ...
        if audience == "auth":
            ...
            return Response(
                {"reply": reply_text, "history": nouvel_historique},
    ...
    try:
        ...
        return Response(
            {"reply": message_reponse.text, "history": nouvel_historique},
            status=status.HTTP_200_OK,
        )
        ...
        if getattr(e, "status_code", None) == 409 or "409" in err_str:
            return Response(
                {
                    "reply": "Conflit : votre demande ne peut être traitée (409). "
                    + CONTACT_MESSAGE
                },
        ...
        return Response(
            {"reply": f"Une erreur est survenue. {CONTACT_MESSAGE}", "error": err_str},
...
def chatbot_user_connected(request):
    ...
    if request.method == "POST":
        ...
        if est_hors_sujet(message_utilisateur):
            reply_text = (
                "En tant qu'assistant AfriCart, je me limite aux sujets liés à la boutique. "
                + CONTACT_MESSAGE
            )
        ...
        reponse_locale = repondre_selon_intention(
            utilisateur, intention, message_utilisateur
        )
        if reponse_locale:
            # respecter l'audience indiquée par l'outil local (auth / anon / all)
            ...
            chatMessage.objects.create(
                utilisateur=utilisateur, role="user", message=message_utilisateur
            )
            chatMessage.objects.create(
        ...
        try:
            ...
            chatMessage.objects.create(
                utilisateur=utilisateur, role="model", message=message_reponse.text
            )
            ...
            if getattr(e, "status_code", None) == 409 or "409" in err_str:
                ...
                chatMessage.objects.create(
                    ...
                    role="model",
                    message="Conflit (409) : demande non traitée",
                )
                return Response(
                    {
                        "reply": "Conflit : votre demande ne peut être traitée (409). "
            ...
            chatMessage.objects.create(
                utilisateur=utilisateur,
                role="model",
                message=f"Une erreur est survenue. {CONTACT_MESSAGE}",
            )
            return Response(
                {
