...
class Vente(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.identifiant_vente:
            ...
            count_today = (
                Vente.objects.filter(date_vente__date=timezone.now().date()).count() + 1
            )
            self.identifiant_vente = f"AfriCart-V-{today_str}-{count_today:03d}"
...
class DetailVente(models.Model):
    identifiant_detail_vente = models.UUIDField(
        default=uuid.uuid4, editable=False, unique=True
    )
    vente = models.ForeignKey(
        Vente, on_delete=models.CASCADE, related_name="details_ventes"
    )
    produit = models.ForeignKey(
        Produit, on_delete=models.PROTECT, related_name="details_ventes"
    )
