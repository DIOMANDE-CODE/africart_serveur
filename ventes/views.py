...
def creer_vente(request):
    ...
    try:
        ...
        if serializer.is_valid():
            ...
            return Response(
                {"message": "Vente créée avec succès", "vente_id": vente.id},
...
def liste_ventes(request):
    try:
        ...
        ventes = Vente.objects.filter(date_vente__date=date.today()).order_by(
            "-date_vente"
        )
    ...
    if user.role != "vendeur":
        ...
        try:
            ventes = Vente.objects.filter(
                date_vente__date=date.today(), utilisateur=user
            ).order_by("-date_vente")
            # Pagination parameters are handled by LimitOffsetPagination
...
def detail_ventes(request, vente_id):
    try:
        ...
        return Response(
            {"success": False, "message": "Vente non trouvée"},
