...
def login_utilisateur(request):
    ...
    try:
        ...
        try:
            ...
            return Response(
                {"success": False, "errors": "Adresse e-mail invalide"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            return Response(
                {"success": False, "errors": "Aucun compte associé à cet e-mail"},
        ...
        if not user.check_password(password):
            return Response(
                {"success": False, "errors": "Mot de passe incorrect"},
        ...
        return Response(
            {"success": False, "errors": "Identifiant invalide"},
            status=status.HTTP_401_UNAUTHORIZED,
        )
...
def login_token(request):
    ...
    try:
        ...
        if user:
            ...
            return Response(
                {"token": token.key, "user_id": user.id, "user": info_user},
                status=status.HTTP_200_OK,
            )
...
def changer_mot_de_passe(request):
    ...
    try:
        user = Utilisateur.objects.get(
            email_utilisateur=user.email_utilisateur, is_active=True
        )
    except Utilisateur.DoesNotExist:
        return Response(
            {"success": False, "errors": "Aucun compte associé à cet e-mail"},
