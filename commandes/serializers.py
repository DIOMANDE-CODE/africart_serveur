...
class CommandeCreateSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        with transaction.atomic():
            ...
            if identifiant_zone:
                zone_obj = ZoneLivraison.objects.filter(
                    identifiant_zone=identifiant_zone
                ).first()
            ...
            if not zone_obj:
                zone_obj = ZoneLivraison.objects.filter(
                    nom_zone__iexact=lieu_livraison
                ).first()
            ...
            if zone_obj:
                ...
                frais_livraison = (
                    Decimal("1000")
                    if "yamoussoukro" in lieu_livraison.lower()
                    else Decimal("3000")
                )
            ...
            for item in items_data:
                ...
                DetailCommande.objects.create(
                    ...
                    sous_total=Decimal(item["prix_unitaire_produit"])
