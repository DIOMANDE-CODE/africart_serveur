...
class ZoneLivraison(models.Model):
    ...
    nom_zone = models.CharField(max_length=50, unique=True) # Ex: "Yamoussoukro" ou "Intérieur"
...
class Commande(models.Model):
    ...
    zone = models.ForeignKey(ZoneLivraison, on_delete=models.SET_NULL, null=True, blank=True, related_name="commandes")
    lieu_livraison = models.CharField(max_length=250, default="yamoussoukro")
    frais_livraison_appliques = models.DecimalField(max_digits=10, decimal_places=2, default=0)
        ...
        if self.latitude_client and self.longitude_client:
            ...
            for z in zones:
                distance = calculer_distance_gps(
                    self.latitude_client, self.longitude_client, 
    ...
    def calculer_totaux(self):
        ...
        self.save(update_fields=['total_ht', 'tva', 'total_ttc', 'frais_livraison_appliques'])
        ...
        if not self.pk:
            ...
            if produit.quantite_produit_disponible < produit.seuil_alerte_produit:
                ...
                AlertProduit.objects.get_or_create(
                    produit=produit,
                    statut_alerte=True,
                    defaults={'message_alerte': f"Stock critique pour {produit.nom_produit}"}
                )
