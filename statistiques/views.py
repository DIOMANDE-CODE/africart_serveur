...
def calculer_apercu_ventes(debut, fin):
    ...
    commandes_livrees = Commande.objects.filter(
        date_commande__date__range=[debut, fin], etat_commande="livre"
    )
    ...
    top_products = (
        DetailVente.objects.filter(vente__date_vente__date__range=[debut, fin])
        .values("produit__nom_produit", "produit__identifiant_produit")
        .annotate(
            total_qty=Sum("quantite"),
            total_amount=Sum(
                F("quantite") * F("prix_unitaire"), output_field=DecimalField()
            ),
        )
    ...
    top_products_sorted = sorted(
        all_products.values(), key=lambda x: x["total_qty"], reverse=True
    )[:5]
...
def calculer_statut_commandes(debut, fin):
    ...
    commandes_en_cours = Commande.objects.filter(
        date_creation__date__range=[debut, fin], etat_commande="en_cours"
    )
    commandes_validees = Commande.objects.filter(
        date_creation__date__range=[debut, fin], etat_commande="valide"
    )
    commandes_livrees = Commande.objects.filter(
        date_creation__date__range=[debut, fin], etat_commande="livre"
    )
    commandes_annulees = Commande.objects.filter(
        date_creation__date__range=[debut, fin], etat_commande="annule"
    )

    total_commande = (
        commandes_en_cours.count()
        + commandes_validees.count()
        + commandes_livrees.count()
        + commandes_annulees.count()
    )
...
def calculer_statut_stock():
    ...
    return {
        ...
        "produits_rupture": [
            {"nom": p.nom_produit, "id": str(p.identifiant_produit)}
            for p in rupture_stock
        ],
...
def calculer_comparaison(period="jour"):
    ...
    evolution_ca = (
        ((ca_actuel - ca_precedent) / ca_precedent * 100) if ca_precedent > 0 else 0
    )
    ...
    ventes_precedentes = Vente.objects.filter(
        date_vente__date__range=[prev_debut, prev_fin]
    ).count()
    evolution_ventes = (
        ((ventes_actuelles - ventes_precedentes) / ventes_precedentes * 100)
        if ventes_precedentes > 0
        else 0
    )
...
def statistiques_quotidiennes_vendeur(request):
    ...
    if user.role != "vendeur":
        ...
        try:
            ...
            ventes_du_aujourdhui = Vente.objects.filter(
                utilisateur=user, date_vente__date=aujourd_hui
            )
            total_ventes_aujourd_hui = sum(
                vente.total_ttc for vente in ventes_du_aujourdhui
            )

            # Somme total des commandes du jour effectuée et livré par le vendeur connecté
            commandes_du_aujourdhui = Commande.objects.filter(
                utilisateur=user, date_commande__date=aujourd_hui, etat_commande="livre"
            )
            total_ventes_commandes_aujourd_hui = sum(
                vente.total_ttc for vente in commandes_du_aujourdhui
            )
            ...
            total_caisse_jour = (
                total_ventes_aujourd_hui + total_ventes_commandes_aujourd_hui
            )

            # Total Commande du jour
            total_commande_aujourd_hui = Commande.objects.filter(
                date_creation__date=aujourd_hui, utilisateur=user
            ).count()

            # Total Commande Attente du jour
            total_commande_attente_aujourd_hui = Commande.objects.filter(
            ...
            # Total Commande Attente du jour
            total_commande_valide_aujourd_hui = Commande.objects.filter(
                date_creation__date=aujourd_hui,
            ...
            total_commande_livre_aujourd_hui = Commande.objects.filter(
                date_creation__date=aujourd_hui, etat_commande="livre", utilisateur=user
            ).count()

            # Total Commande Annulée du jour
            total_commande_annulee_aujourd_hui = Commande.objects.filter(
                date_creation__date=aujourd_hui,
