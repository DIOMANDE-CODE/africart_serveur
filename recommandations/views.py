...
def enregistrer_vue_produit(request):
    ...
    if not produit_id:
        return Response(
            {"error": "produit_id requis"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    VueProduit.objects.create(
        produit=produit, utilisateur=utilisateur, session_key=session_key
    )
...
def obtenir_recommandations(request):
    ...
    if type_reco not in types_valides:
        return Response(
            {
                "error": f"type de recommandation invalide. Choisissez parmi : {', '.join(types_valides)}."
            },
    ...
    if type_reco == "personnalise":
        ...
        return Response(
            {
                "type": "personnalise",
                "data": {"produits": _personnalise(request.user, limite)},
            },
    ...
    if type_reco == "best_sellers":
        return Response(
            {"type": "best_sellers", "data": {"produits": _best_sellers(limite)}},
            status=status.HTTP_200_OK,
        )
    ...
    if not produit_id:
        return Response(
            {"error": "produit_id requis pour ce type de recommandation."},
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    recos = (
        Recommandation.objects.filter(
            produit_source=produit_src, type_recommandation=type_reco
        )
    ...
    return Response(
        {"type": type_reco, "data": {"produits": produits}}, status=status.HTTP_200_OK
    )
...
def _best_sellers(limite):
    recos = (
        Recommandation.objects.filter(
            produit_source__isnull=True, type_recommandation="best_sellers"
        )
        .select_related("produit_recommande__categorie_produit")
        .order_by("-score")[:limite]
    )
    ...
    cache = {
        p.pk: p
        for p in Produit.objects.filter(pk__in=ids).select_related("categorie_produit")
    }
    return [
        _ser(cache[v["produit_id"]], v["total"])
        for v in ventes
        if v["produit_id"] in cache
    ]


def _personnalise(utilisateur, limite):
    deja_achetes = set(
        DetailCommande.objects.filter(commande__utilisateur=utilisateur).values_list(
            "produit_id", flat=True
        )
    )
    recos_co = list(
        Recommandation.objects.filter(
            produit_source__in=deja_achetes, type_recommandation="co_achat"
        )
    ...
    vus = set(
        VueProduit.objects.filter(
            utilisateur=utilisateur, timestamp__gte=timezone.now() - timedelta(days=30)
        )
    ...
    recos_sim = list(
        Recommandation.objects.filter(
            produit_source__in=vus, type_recommandation="similaire_categorie"
        )
        .exclude(produit_recommande__in=deja_achetes)
        .select_related("produit_recommande__categorie_produit")
        .order_by("-score")[:limite]
    ...
    for r in recos_co + recos_sim:
        ...
        if len(fusionnes) >= limite:
            break
    return (
        [_ser(r.produit_recommande, r.score) for r in fusionnes]
        if fusionnes
        else _best_sellers(limite)
    )
...
def _ser(produit, score=None):
    return {
        ...
        "prix_promo": (
            float(produit.prix_promo_produit) if produit.prix_promo_produit else None
        ),
        "thumbnail": produit.thumbnail,
        "categorie": (
            produit.categorie_produit.nom_categorie
            if produit.categorie_produit
            else None
        ),
